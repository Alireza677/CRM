    ...
    <div>
        <div class="px-4">
            <div class="flex flex-col gap-3 mb-6">
                <div>
                    <h2 class="text-2xl font-semibold text-gray-800">لیست سرنخ‌های منتخب</h2>
                    <p class="text-sm text-gray-500 mt-1">
                        در این بخش می‌توانید سرنخ‌هایی که به عنوان منتخب ذخیره کرده‌اید را مشاهده و مدیریت کنید.
                    </p>
                </div>
            ...
            @if($leads->count() === 0)
                <div class="bg-white border rounded-lg p-6 text-center text-sm text-gray-500">
                    <p>در حال حاضر هیچ سرنخ منتخبی ثبت نشده است.</p>
                    <p class="mt-2">
                        برای افزودن به لیست منتخب‌ها، از صفحه‌ی نمایش سرنخ یا از لیست سرنخ‌ها آن را به منتخب اضافه کنید.
                    </p>
                </div>
            @else
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-2 py-2 text-right">نام سرنخ</th>
                            ...
                            @foreach($leads as $lead)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-4 py-2 text-center">
                                        <form method="POST"
                                              action="{{ route('marketing.leads.favorites.destroy', $lead) }}"
                                              class="inline-flex"
                                              onsubmit="return confirm('آیا از حذف از علاقه مندی مطمئن هستید؟');">
                                            @csrf
                                            @method('DELETE')
