...
if (applyBtn) {
    ...
    // ذخیره username ها در input مخفی برای پردازش
    ...
    if (e.target.classList.contains('remove-mention')) {
        ...
        // حذف input مخفی مربوط به این user{id}
        document.querySelectorAll(`input[name="mentions[]"]`).forEach(input => {
            if (input.value === `user${userId}`) {
                input.remove();
            }
        });
        ...
        const updatedUsers = remainingInputs.map(input => {
            const id = input.value.replace('user', '');
            const cb = document.querySelector(`.mention-checkbox[data-id="${id}"]`);
            return {
                id,
        ...
        if (selectedMentions) {
            selectedMentions.innerHTML = updatedUsers.map(u => `
                <span class="inline-flex items-center bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full text-xs ml-1 mb-1">
                    ${u.name}
                    <button type="button" class="ml-[5px] text-red-600 hover:text-red-800 font-bold remove-mention" data-id="${u.id}">&times;</button>
                </span>
            `).join(' ');
        }
    }
