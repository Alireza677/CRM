...
<div class="space-y-4">
                    ...
                    @foreach($attributes as $key => $newVal)
                        @php
                            ...
                            if ($key === 'organization_id') {
                                $oldVal = \App\Models\Organization::find($oldVal)?->name ?? $oldVal;
                                $newVal = \App\Models\Organization::find($newVal)?->name ?? $newVal;
                            }
    ...
    @empty
        <div class="text-sm text-gray-500">به‌روزرسانی‌ای برای این مخاطب ثبت نشده است.</div>
    @endforelse
</div>
