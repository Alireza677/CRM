            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('forms.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input id="title" class="block mt-1 w-full border-gray-300 shadow-sm rounded-md focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="text" name="title" value="{{ old('title') }}" required autofocus>
                        @error('title')
                            <p class="text-sm text-red-600 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="purchase_date" class="block mt-1 w-full border-gray-300 shadow-sm rounded-md focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="date" name="purchase_date" value="{{ old('purchase_date') }}" required>
                        @error('purchase_date')
                            <p class="text-sm text-red-600 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="total" class="block mt-1 w-full border-gray-300 shadow-sm rounded-md focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="number" name="total" value="{{ old('total') }}" required>
                        @error('total')
                            <p class="text-sm text-red-600 mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center justify-end mt-4">
                        ...
                        <button type="submit" class="mr-3 inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700">
                            ایجاد فرم
                        </button>
                    </div>
                </form>
